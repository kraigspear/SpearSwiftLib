...
public extension UIView {
    ...
    func animateScale(withDuration: TimeInterval,
                      from: Double = 0.85,
                      to: Double = 1.0) {
        let animateKey = "scale"
        let animateScale = CABasicAnimation(keyPath: "transform.scale")
        animateScale.duration = withDuration
        animateScale.fromValue = from
        animateScale.toValue = to
        animateScale.autoreverses = true
        animateScale.repeatCount = Float.greatestFiniteMagnitude
        layer.add(animateScale, forKey: animateKey)
    }
