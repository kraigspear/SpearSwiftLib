...
public final class ImageCache {
    ...
    private func fetchFromMemory(_ urlStr: String) -> UIImage? {
        return memoryCache.object(forKey: urlStr as AnyObject) as? UIImage
    }

    private func fetchFromDisk(_ urlStr: String) -> UIImage? {
        let fileNameCache = fileNameFromCache(urlStr)

        if fileExistInCacheDirectory(urlStr) {
            if let imageFromCache = UIImage(contentsOfFile: fileNameCache) {
                memoryCache.setObject(imageFromCache, forKey: urlStr as AnyObject)
        ...
        if let url = URL(string: urlStr) {
            if let imageData = try? Data(contentsOf: url) {
                if let imageFromURL = UIImage(data: imageData) {
                    memoryCache.setObject(imageFromURL, forKey: fileForUrl(urlStr) as AnyObject)
    ...
    private func cacheDirectoryAppendFileName(_ fileName: String) -> String {
        return cacheDirectory.appendingPathExtension(fileName)!
    }
