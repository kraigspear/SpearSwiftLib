...
public class ImageCache
{
  ...
  private func fetchFromUrl(urlStr: String) -> UIImage?
  {
    ...
    public func fetchImageFakeScale(urlStr:String, completed:ImageBlock)
    {
        ...
        let nsIconStr = NSString(string: nsUrlStr)
        
        let nsLastPath = NSString(string:nsIconStr.lastPathComponent)
        let fileName = nsLastPath.stringByDeletingPathExtension + "@2x.png"
        let pathFile = self.cacheDirectory.stringByAppendingPathComponent(fileName)
        
        if let unwrapCompleted = completed
        {
            dispatch_async(dispatch_get_global_queue(Int(QOS_CLASS_USER_INITIATED.rawValue), 0)) {[weak self] () -> Void in
                
                if let unwrapSelf = self
                {
                    if let image = unwrapSelf.memoryCache.objectForKey(fileName) as? UIImage
                    {
                        dispatch_async(dispatch_get_main_queue())
                        {
                    ...
                    {
                        if let image = UIImage(contentsOfFile: pathFile)
                        {
                            unwrapSelf.memoryCache.setObject(image, forKey: fileName)
                            dispatch_async(dispatch_get_main_queue())
                            {
                               unwrapCompleted(image:image)
                            }
                    ...
                    if let nsUrl = NSURL(string: urlStr)
                    {
                        ...
                        {
                            data.writeToFile(pathFile, atomically: false)
                            if let image = UIImage(contentsOfFile: pathFile)
                            {
                                unwrapSelf.memoryCache.setObject(image, forKey: fileName)
                                dispatch_async(dispatch_get_main_queue())
  ...
  All loading is done on a separate thread, so the caller doesn't have to worry about createing one.
  The callback (completed) is done back on the main thread.
  
  :param: urlStr    A url string where the file can be downloaded
  :param: completed Called with a valid image or nil, if the file can't be loaded
  */
